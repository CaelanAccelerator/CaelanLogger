...
template <typename T>
class SPMCSpinLockQueue : public ConcurrentRingBuffer<SPMCSpinLockQueue<T>, T>
{
public:
  using Base = ConcurrentRingBuffer<SPMCSpinLockQueue<T>, T>;

  explicit SPMCSpinLockQueue(size_t bufferSize) : Base(bufferSize) {}
...
template <typename T>
bool SPMCSpinLockQueue<T>::pushImpl(T &&elem)
{
...
template <typename T>
bool SPMCSpinLockQueue<T>::pushImpl(const T &elem)
{
  SpinGuard guard(spinlock_);
  size_t h = this->head_.load(std::memory_order_relaxed);
  if (h - this->tail_.load(std::memory_order_acquire) == this->capacity_)
...
template <typename T>
std::optional<T> SPMCSpinLockQueue<T>::popImpl()
{
  SpinGuard guard(spinlock_);
