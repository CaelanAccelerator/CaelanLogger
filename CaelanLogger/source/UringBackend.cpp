...
UringBackend::UringBackend(size_t bufferSize, size_t poolCapacity,
                           std::string dir, io_uring *ring)
    : FileUtil<UringBackend>(dir), poolCapacity_(poolCapacity),
...
void UringBackend::record_drop() { add_dropped(); }
...
void UringBackend::writeDropMessage(char *msg, int len) {
  ...
  pendingCloseFd_ = fd_; // remember which fd this message actually belongs to;
                         // fd_ itself will be reassigned to the new file right
                         // after this call
...
void UringBackend::reapCompletion(io_uring_cqe *cqe) {
  if (msgBuffer_ != nullptr &&
      reinterpret_cast<char *>(cqe->user_data) == msgBuffer_) {
    delete[] msgBuffer_;
    msgBuffer_ = nullptr;
...
void UringBackend::submit(std::unique_ptr<Buffer> fullBuffer) {
  ...
  if (shouldRoll(bufferPool_[idxIn]->size())) {
    roll();
  }

  io_uring_sqe *sqe = io_uring_get_sqe(ring_);
  if (!sqe) {
    add_dropped(bufferPool_[idxIn]->lineCount());
    bufferPool_[idxIn]->reset();
    freeIdxes_->push(idxIn);
    return;
  }

  // give the fullBuffer to io_uring
  writtenBytes += bufferPool_[idxIn]->size();
  io_uring_prep_write(sqe, fd_, bufferPool_[idxIn]->getBuffer(),
                      bufferPool_[idxIn]->size(), -1);
  io_uring_sqe_set_data(sqe, reinterpret_cast<void *>(idxIn));
  io_uring_submit(ring_);
}
