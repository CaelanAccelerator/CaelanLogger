...
recent_files(const fs::path &dir, fs::file_time_type start_time)
{
...
static std::size_t count_token_occurrences(const fs::path &p, const std::string &token)
{
    ...
    while (in)
    {
        ...
        if (token.size() > 1)
        {
            ...
            if (view.size() > keep)
                carry.assign(view.end() - keep, view.end());
            else
                carry = view;
        }
        else
                                      ...
                                      const std::vector<std::string> &tokens)
{
    for (const auto &t : tokens)
        cleanup_files_with_token(logDir, start_time, t);
}
...
{
    if (worst_prefix.size() + extra_suffix_bytes >= maxLine)
        return 0;
